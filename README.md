# PHP PSR-15: HTTP Request Handlers & Middleware

This library provides a collection of standardized, single-purpose PSR-15 middlewares and request handlers. It includes essential infrastructure components for error handling, exception management, content negotiation, and response emission, designed to be composed into a linear request processing pipeline.

Each handler is implemented as a final, stateless class with strict dependencies. The architecture encourages a "pipeline" design pattern where request processing is broken down into atomic, testable units, ensuring high maintainability and ease of debugging in production environments.
